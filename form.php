...
function showNotesModal(contentHtml) {
  ...
  const openSnapshot = stackedModalOrder.slice();
  openSnapshot.forEach(modal => {
    if (!modal || modal === notesModal) {
      return;
    }
    if (!suppressedSet.has(modal)) {
      suppressedSet.add(modal);
  ...
  allModals.forEach(modal => {
    if (!modal || modal === notesModal || suppressedSet.has(modal)) {
      return;
    }
    const computed = window.getComputedStyle ? window.getComputedStyle(modal) : null;
    const currentlyVisible = computed ? computed.display !== 'none' : modal.style.display !== 'none';
    if (!currentlyVisible) {
      return;
    }

    suppressedSet.add(modal);
    suppressedForNotes.push({
      modal,
      viaStack: false,
      previousDisplay: modal.style.display || ''
    });

    modal.dataset.notesSuppressed = 'true';
...
function hideNotesModal() {
  ...
  while (suppressedForNotes.length) {
    const entry = suppressedForNotes.shift();
    ...
    if (entry.viaStack) {
      ...
      showStackedModal(entry.modal);
      if (entry.previousDisplay) {
        entry.modal.style.display = entry.previousDisplay;
      } else {
        entry.modal.style.removeProperty('display');
      }
      delete entry.modal.dataset.notesSuppressed;
    }
  }
}
